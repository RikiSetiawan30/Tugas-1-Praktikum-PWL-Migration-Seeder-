...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-xl font-semibold">Daftar Rak Buku</h1>
    <div class="flex gap-2">
        <a href="{{ route('bookshelfs.export-pdf') }}" class="px-4 py-2 bg-gray-600 text-white text-sm rounded-md hover:bg-gray-500">Export PDF</a>
        <a href="{{ route('bookshelfs.export-excel') }}" class="px-4 py-2 bg-gray-600 text-white text-sm rounded-md hover:bg-gray-500">Export Excel</a>
        <a href="{{ route('bookshelfs.create') }}" class="px-4 py-2 bg-gray-800 text-white text-sm rounded-md hover:bg-gray-700">Tambah</a>
    </div>
</div>
...
<div class="mt-4 bg-white border border-gray-200 rounded-lg p-4 max-w-sm">
    ...
    <form action="{{ route('bookshelfs.import') }}" method="POST" enctype="multipart/form-data" class="flex gap-2">
        @csrf
...
<div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
    <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            @forelse($bookshelfs as $i => $shelf)
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 text-gray-500">{{ $bookshelfs->firstItem() + $i }}</td>
                <td class="px-4 py-3 font-mono">{{ $shelf->code }}</td>
                <td class="px-4 py-3">{{ $shelf->name }}</td>
                <td class="px-4 py-3 flex gap-2">
                    <a href="{{ route('bookshelfs.edit', $shelf) }}"
                       class="px-3 py-1 text-xs border border-gray-300 rounded hover:bg-gray-100">Edit</a>
                    <form action="{{ route('bookshelfs.destroy', $shelf) }}" method="POST" class="inline">
                        @csrf @method('DELETE')
                        <button type="submit"
